...
if ~isfile(f2)
    ...
    emit(['Interpretation: r_E3 (true KE delivered / stored) is tiny (~0.4%% stress, ~0.2%% nominal) ' ...
          'and FLAT across the mix -> H_load is a consistent ~250x overestimate of delivered inertia, ' ...
          'set by the frequency-excursion coupling not the H distribution. r_E1 (RoCoF-apparent) ' ...
          'EXCEEDS 1 and drifts down with H_load (corr<0): it is dominated by the motors'' fast ' ...
          'frequency response / load relief (a damping term, per E2''s ~0 inertia coefficient at low ' ...
          'condition number), NOT synchronous inertia. So H_load bounds delivered inertia but NOT ' ...
          'the RoCoF-measured value. See results_effective_inertia.md for the full write-up.']);
end
emit('');
